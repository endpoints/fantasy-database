# this is the same as sequel_polymorphic except that it uses lowercased, plural
# names by default for the polymorphic type name
# polymorphic data is horrble. don't do it.
        ...
        def one_to_many(*args, &block)
          ...
          if able = options[:as]
            ...
            associate(:one_to_many, collection_name,
              :key        => able_id,
              :reciprocal => able,
              :conditions => {able_type => pluralize(self.to_s.downcase)},
        ...
        def many_to_many(*args, &block)
          ...
          if through = (options[:through] || options[:join_table]) and able = options[:as]
            ...
            associate(:many_to_many, collection_name,
              :left_key   => able_id,
              :join_table => through,
              :conditions => {able_type => self.to_s.downcase},
